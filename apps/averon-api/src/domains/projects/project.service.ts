...
export class ProjectService {
  ...
  async activateFromPayment(payment: PaymentEvent) {
    ...
    if (await this.repository.findByContract(payment.contractId)) return "duplicate" as const;
    const password = generateProjectAccessPassword();
    const hash = await hashProjectAccessPassword(password);
    const project = await this.repository.createFromPaidContract({ eventId: payment.eventId, customerId: payment.customerId, contractId: payment.contractId, invoiceId: payment.invoiceId }, hash);
  ...
  async regenerate(id: string) {
    ...
    } catch { await this.repository.markEmail(id, "failed"); }
    return { projectId: id, regenerated: true };
  }
}
